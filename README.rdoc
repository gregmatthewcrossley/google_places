...
To use this gem, install it with <tt>gem install google-places</tt> or add it to your Gemfile:
...
### Testing
The test suite needs a real working API key to function. Copy `spec/api_key.sample.rb` to `spec/api_key.rb` and update `abc123` to your API key. Once that's done, run:
```shell
bundle exec rspec
```